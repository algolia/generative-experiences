...
import { GenerationSource, ProductsComparisonOptions, RequestParameters, ShoppingGuide, ShoppingGuideContentOptions, ShoppingGuideHeadline, ShoppingGuideHeadlinesOptionsForCombined, ShoppingGuideHeadlinesOptionsForGenerated, ShoppingGuideHeadlinesOptionsForIndex } from './types';
...
type ShoppingGuideTaskArgs = TaskBaseArgs & { search_parameters?: PlainSearchParameters }
...
export function _createClientBase(
    ...
    return {
        ...
        async getOrGenerateHeadlines<TSource extends GenerationSource>(
            ...
            if (source === 'combined' || source === 'generated') {
                if (params.source === 'combined') {
                    const { generateParams, ...combinedParams } =
        ...
        async vote({ objectIDs, voteType, userToken, voteTarget }: {
            objectIDs: string[];
